...
function gameController(idx) {
  if (game.isPlacingFlags && !grid.isRevealed[idx[0]][idx[1]]) {
    //place flag on grid
  ...
  } else if (game.isPlacingQs && !grid.isRevealed[idx[0]][idx[1]]) {
    //place ? on grid
  ...
  } else if ((!grid.isRevealed[idx[0]][idx[1]]) || debug) {
  ...
  if (symb === 'X') {
    ...
    for (adjGridIdx of adjGrid) {
      if ((grid.matrix[adjGridIdx[0]][adjGridIdx[1]] != 'X') && !grid.isRevealed[adjGridIdx[0]][adjGridIdx[1]]) {
        ...
        grid.specialIcons[adjGridIdx[0]][adjGridIdx[1]] = '';
...
function clickTracker(event) {
  ...
  } else if (((evEl.classList.contains('minesweeperGrid') || evEl.classList.contains('mine')) && !game.end) ) {
  ...
  for (i in grid.matrix) {
    for (j in grid.matrix[i]) {
      ...
      if (grid.isRevealed[i][j]) {
        ...
        gridEl.innerText = grid.specialIcons[i][j]; //display flag or ?
...
function renderGameHeader() {
  //Update game clock
  if ((game.safeSquaresRevealed > 0) && !game.end) {
    //no need to update the clock before the player's first move or if the game is over
    updateGameClock();
  }

  //visually update toggles
  let bg = '';
